  ...
  it.each(['active', 'public', 'invalid'])(
    ...
    async (status) => {
      ...
      try {
        ...
        expect(wrapper.get('.customers__name').element.tagName).toBe('A')
        expect(wrapper.find('[aria-label="筛选客户"]').exists()).toBe(true)
        expect(wrapper.get('.customers__pool-row').text()).toContain('在案客户')
